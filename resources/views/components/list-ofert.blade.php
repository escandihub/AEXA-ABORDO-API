...
<div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100" x-data="BrandList('{{ $wireModel }}')">
    <div class="flex flex-col gap-4">
        ...
        <ul class="select-none grid w-full gap-4 md:grid-cols-3">
            ...
            <li >
                <input type="radio" :id="'option-'brand.name" :value="brand.name" :name="brand.name" class="hidden peer"
                    required="" wire:model="{{ $wireModel }}">
                <label :for="brand.name" @click="emitOrSet(brand.name)"
                    ...
                    <div class="block">
                        <svg class="mb-2 w-7 h-7 text-sky-600" fill="currentColor" aria-hidden="true"
                        ...
                        <div class="w-full font-medium mb-1" x-text="brand.name"> </div>
