@props([
    ...
    'emitSuccessUpload' => false
])

<div x-data="{ 
    isMultiple: Boolean('{{ $multiple }}') || false, 
    emitSuccess: Boolean('{{ $emitSuccessUpload }}') || false, 
    progress: 0,
    isFocused: false,
    handleFiles() { 
        if (this.isMultiple === true) {
            ...
            @this.upload('{{ $attributes->wire('model')->value }}',  this.$refs.input.files[0], (uploadedFilename) => {
                if(this.emitSuccess){
                    let name = @entangle($attributes->wire('model')) ;
                    $wire.emit('full-upload', name.livewireEntangle)
                }
            }, () => {
            }, (event) => {
                this.progress = event.detail.progress || 0
    ...
    uploadSingle(){
        @this.upload('singlefile'), this.$refs.input2.file,
    ...
    @if ($mode === 'attachment')
        ...
        <div>
            @if (is_array($file) && count($file) > 0)
                ...
                @endforeach
                <input type="file" id="file-520"  wire:model="file" x-ref="input2"/>
                <button wire:click.prevent="addFile" class="bg-blue-300 text-blue-500 hover:bg-blue-100 rounded-md px-3">Agregar</button>
            @else
                @if ($file)
                    <div class="mt-3 flex">
                        <div class="w-16 mr-4 flex-shrink-0 shadow-xs rounded-lg">
                            @if (collect(['csv'])->contains($file->getClientOriginalExtension()))
                        ...
                        <div>
                            ...
                            <div class="flex items-center space-x-1">
                                ...
                                <div class="text-xs text-gray-500 uppercase">
                                    {{ $file->getClientOriginalExtension() }}</div>
                            </div>
